...
/**
 * Stats data by day model class
 *
 * @package    report_alirostats
 * @copyright  2017 Kota Kobayashi
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class stats_data_byday extends stats_data {
    /**
    ...
    public function analyze_access_records() {
        foreach (parent::get_records() as $record) {
            ...
            $this->access_numbers[$record->courseid][date('w', $record->timecreated)]++;
        }
        $this->store_access_numbers('day');
    }
    ...
    public function init_access_numbers() {
        foreach (parent::get_courses() as $course) {
            $this->access_numbers[$course->id] = array_fill(0, 7, 0);
        }
    }
}
